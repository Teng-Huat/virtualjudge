defmodule A2OJ.Scraper do
  alias VirtualJudge.Problem
  alias VirtualJudge.Repo

  @doc """
  Scrapes A2OJ `url` for the specific problem, saves it to the database
  ...
  def perform(url, topic) do
    ...
    [_full_match, url_path] = Regex.run(~r/^https:\/\/.*\.com(.*)/, url)

    A2OJ.start()

    cookies = A2OJ.login(username, password)

    {title, description, language, source} = A2OJ.scrape_problem(url_path)
